...
async function deploy() {
  ...
  if (testMode) {
    ...
    var dex_path: string;
    const root_main = "/althea/solidity-dex/artifacts/contracts/";
    const main_location_a = root_main + "CrocSwapDex.sol/CrocSwapDex.json";

    const root_alt_1 = "/solidity-dex/";
    const alt_location_1_a = root_alt_1 + "CrocSwapDex.json";

    const alt_location_2_a = "CrocSwapDex.json";

    var contract_paths: CrocPaths;
    if (fs.existsSync(main_location_a)) {
      contract_paths = get_paths(root_main, true);
    } else if (fs.existsSync(alt_location_1_a)) {
      contract_paths = get_paths(root_alt_1, false);
    } else if (fs.existsSync(alt_location_2_a)) {
      contract_paths = get_paths("", false);
    } else {
      console.log(
        "Test mode was enabled but the DEX contracts can't be found!"
      );
      exit(1);
    }
    ...
    console.log("Deploying CrocSwap contracts");
