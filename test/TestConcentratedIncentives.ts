...
describe('ConcentratedIncentives', () => {
    let test1: TestPool
    let test2: TestPool
    let test3: TestPool
    ...
    it("mint, register, withdraw rewards from multiple pools", async () => {
        ...
        let liq2 = 2000000;
        await test2.testMint(-5000, 8000, liq2); 
        ...
        await incentives.registerForConcentratedRewards(poolId1, rewardToken.address); // 1-0 2-?
        ...
        await incentives.registerForConcentratedRewards(poolId2, rewardToken.address); // 1-10 2-0
    ...
    it("mint, register, withdraw rewards from multiple incentives contracts", async () => {
        ...
        await incentives2.createOrModifyConcentratedRewardsProgram(
            baseQuotePoolId, 
            rewardToken.address, 
            rateNum2,  
            rateDen2, 
        ...
        await incentives.registerForConcentratedRewards(baseQuotePoolId, rewardToken.address); // 1-0 2-?
        await incentives2.registerForConcentratedRewards(baseQuotePoolId, rewardToken.address); // 1-1 2-0
    ...
    it("randomized users with positions in the incentivized pool", async () => {
        ...
        for (let i = 0; i < numUsers; i++) {
            ...
            await test1.base.contract.deposit(user.address, finalPosLiq);
            await test1.quote.contract.deposit(user.address, finalPosLiq);
            await test1.base.contract.connect(user).approve((await test1.dex).address, finalPosLiq);
            await test1.quote.contract.connect(user).approve((await test1.dex).address, finalPosLiq);
            await test1.testMintFrom(user, -5000, 8000, liq); 

            liqPositions.push(finalPosLiq);
            await incentives.connect(user).registerForConcentratedRewards(baseQuotePoolId, rewardToken.address);
