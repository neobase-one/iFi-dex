...
contract StorageLayout {
     ...
     *  @param paidInBase The amount of base token fees earned by the pool.
     *  @param paidInQuote The amount of base token fees earned by the pool.
    ...
    // The total amount of liquidity removed from a pool. This value is incremented any time ambient liquidity is removed from a pool
    // @note since fees are collected as ambient liquidity removed must always be less than or equal to added.
    // @note this value is denominated normalized liquidity units
    ...
    /** @dev This accumulator represents the total amount of fees generated by a specific LP owner in a pool and can be compared to the
     * total fees generated by the pool.
     * @dev this value is updated only when the user claims their fees and does not include ambient liquidity fees.
     * @dev this value is normalized as the geometric mean of the two fee values */
     ...
     *  @param feeLiq The amount of liquidity units (sqrt(X * Y)) corresponding to fees earned by the user.
     *  @param poolId The pool who's fee value will be updated.
     */
    function updateIncentiveUserPoolFeeAccumulator(address user, uint128 feeLiq, bytes32 poolId) internal {
        incentiveUserPoolFeeAccumulators_[user][poolId] += feeLiq;
    }
