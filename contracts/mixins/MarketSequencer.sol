...
contract MarketSequencer is TradeMatcher {
    ...
    function applySwap (TokenFlow.PairSeq memory pairs,
        ...
        if (dir.qty_ != 0) {
            (int128 paidInBase, int128 paidInQuote) = callSwap(flow, curve, dir, cntx.pool_);
            // update the pool fee accumulator, this data is public and can be used by external rewards
            // contracts to provide incentives for liquidity that are proportinal to the swap fees generated by a position
            // in particular, this provides a total to figure a particular position's share of fees generated
            updateIncentivePoolFeeAccumulator(paidInBase, paidInQuote, cntx.pool_.hash_);
            if (dir.isBuy_) {
    ...
    function applyAmbient (TokenFlow.PairSeq memory pairs,
        ...
        if (dir.liquidity_ > 0) {
            ...
            if (dir.isAdd_) {
                ...
                updateIncentivePoolAmbLiqAddedAccumulator(dir.liquidity_, cntx.pool_.hash_);
                updateIncentiveUserPoolAmbLiqAddedAccumulator(lockHolder_, dir.liquidity_, cntx.pool_.hash_);
            } else {
                ...
                emit CrocEvents.BurnAmbient(lockHolder_, pairs.baseToken_, pairs.quoteToken_,
                                            poolIdx, dir.liquidity_, base, quote);
                // update incentives trackers, liq is normalized liquidity
                updateIncentivePoolAmbLiqRemovedAccumulator(dir.liquidity_, cntx.pool_.hash_);
                updateIncentiveUserPoolAmbLiqRemovedAccumulator(lockHolder_, dir.liquidity_, cntx.pool_.hash_);
            }
    ...
    function applyConcentrated (CurveCache.Cache memory curve,
        ...
        if (bend.isAdd_) {
            ...
            // update incentives trackers, liq is normalized liquidity
            updateIncentivePoolConcLiqAddedAccumulator(bend.liquidity_, cntx.pool_.hash_);
            updateIncentiveUserPoolConcLiqAddedAccumulator(lockHolder_, bend.liquidity_, cntx.pool_.hash_);
            ...
            // update incentives trackers, liq is normalized liquidity
            updateIncentivePoolConcLiqRemovedAccumulator(bend.liquidity_, cntx.pool_.hash_);
            updateIncentiveUserPoolConcLiqRemovedAccumulator(lockHolder_, bend.liquidity_, cntx.pool_.hash_);
            // Update the user's fees accumulator, rewards is liquidity removed to pay them fees
            updateIncentiveUserPoolFeeAccumulator(lockHolder_, rewards, cntx.pool_.hash_);
